...
const User = ({ user }: { user: PeerDetailsType }) => {
  return (
    <div className="flex items-center gap-2 w-full py-3 px-3">
      {user.image ? (
        <Avatar  src={user.image} size="md" className="select-none" />
      ) : (
        <Avatar  name={user.name} size="md" className="select-none" />
      )}
      <div className="flex flex-col flex-1 select-none">
        <div className="name font-medium text-ellipsis flex-1">{user.name}</div>
      </div>
      <Sound />
          ...
          <div className="meetingpeople px-3 pb-2 flex flex-col w-full">
            <div className="users border-[1.5px] border-gray-400/30 rounded-lg w-full mt-1.5 flex-1 flex flex-col overflow-y-scroll no-scrollbar">
              ...
              >
                ...
                Adipisci ad enim quaerat at odit. Ullam quas totam
                necessitatibus suscipit, beatae eveniet. Quo nam beatae iste
                magni odit porro eum, dolores amet maiores ullam eligendi
                numquam in ducimus ad exercitationem sit, odio ratione quidem
                quia enim, at distinctio accusamus! Impedit eveniet id atque
                quidem. Molestiae reprehenderit magni voluptatem, earum animi,
                aperiam natus error incidunt voluptas soluta, ab sunt! Maxime
                facere delectus nihil voluptatibus odit voluptate aperiam quam
                qui temporibus fuga nemo, veniam corrupti! Praesentium
                repudiandae provident laudantium eius temporibus architecto
                natus! Dolore velit aut voluptas animi vel atque natus
                cupiditate qui.
              </motion.div>
            </div>
          </div>
